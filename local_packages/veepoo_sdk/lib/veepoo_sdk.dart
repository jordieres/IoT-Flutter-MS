...
class VeepooSdk {
  ...
  static Future<List<SearchResult>> scanDevice() async {
    ...
    return devices.map((deviceJson) {
      ...
      Map<String, dynamic> castedMap = Map<String, dynamic>.from(deviceJson as Map);
  ...
  static Future<bool?> bind(String password, bool is24h) async {
    return await _channel.invokeMethod('confirmDevicePwd', {'password': password, 'is24h': is24h});
  }
  ...
  static Future<String?> startDetectBP() async {
    final String? message = await _channel.invokeMethod('startDetectBP');
    return message;
  }
  ...
  static Future<Map<String, dynamic>?> readStepData() async {
    final Map<dynamic, dynamic>? stepData = await _channel.invokeMethod('readStepData');
    return stepData?.cast<String, dynamic>();
  }

  // ----------------------------------------------------------------------------------------------
  // Read data

  static Future readFiveMinutes() async {
    await _channel.invokeListMethod('readFiveMinutes');
  }
  ...
  static Future<List<HRVOriginData>> readHRVOrigin() async {
    String rawJson = await (_channel.invokeMethod('readHRVOrigin') as FutureOr<String>);
  ...
  static Future<OriginV0Data> readOriginData() async {
    String rawJson = await (_channel.invokeMethod('readOriginData') as FutureOr<String>);
    return OriginV0Data.fromJson(json.decode(rawJson));
  }

  static Future<OriginV3Data> readOrigin3Data() async {
    String rawJson = await (_channel.invokeMethod('readOrigin3Data') as FutureOr<String>);
  ...
  static Future<List<EcgDetectResult>> readECGData() async {
    String rawJson = await (_channel.invokeMethod('readECGData') as FutureOr<String>);
  ...
  static Future<List<Spo2hOriginData>> readSpo2hOrigin() async {
    String rawJson = await (_channel.invokeMethod('readSpo2hOrigin') as FutureOr<String>);
    List<dynamic> raw = json.decode(rawJson);
  ...
  static Future<List<SleepData>> readSleepData() async {
    String rawJson = await (_channel.invokeMethod('readSleepData') as FutureOr<String>);
    List<dynamic> raw = json.decode(rawJson);
